...
export const myFeature = defineFeature(function(context is Context, id is Id, definition is map)
    ...
    }
    {
        ...
        var emblem_s1 = [vector(398, 904) * millimeter, vector(484, 785) * millimeter, vector(572, 682) * millimeter];
        var emblem_s2 = [vector(572, 682) * millimeter, vector(516, 620) * millimeter, vector(460, 540) * millimeter];
        var emblem_s3 = [
        ...
        var emblem_s8 = [vector(435, 265) * millimeter, vector(456, 244) * millimeter, vector(479, 226) * millimeter];
        var emblem_s9 = [
        ...
        //to specify the right plane when the user selects on a 3D model's face (for instance), vectors were used instead.
        var emblem_right = [emblem_s1, emblem_s2, emblem_s3, emblem_s4, emblem_s5, emblem_s6, emblem_s7, emblem_s8, emblem_s9];
        var emblem_left = [emblem_s1, emblem_s2, emblem_s3, emblem_s4, emblem_s5, emblem_s6, emblem_s7, emblem_s8, emblem_s9];

        emblem_right = shiftFlipScale(context, emblem_right, definition.scale, -398, -504, 0);
        emblem_left = shiftFlipScale(context, emblem_left, definition.scale, 398, -504, 1);
        
        //Creates the 2D sketch
        sketchEmblem(context, id, emblem_left, emblem_right, definition.parameter);
    ...
    //shiftAndScale :
        ...
        //scale_factor -> enlarge or decrease size of emblem (defined by user)
        //shift_x -> hard coded for shifting first point in emblem_s1 to be aligned with vertical axis
        //shift_y -> hard coded for shifting first point in emblem_s1 so entire emblem is centered around the origin
        //flip -> flag to determine if coordinates need to be flipped across vertical axis
    function shiftFlipScale(context is Context, emblem is array, scale_factor is number, shift_x is number, shift_y is number, flip is number) {
        for (var i = 0; i != 9; i += 1) {
            for (var j = 0; j != size(emblem[i]); j += 1) {
                ...
                emblem[i][j][0] += shift_x * millimeter;
                emblem[i][j][1] += shift_y * millimeter;
                emblem[i][j] *= scale_factor / 40;
            }
        }
        return (emblem);
    }
    ...
    //sketchEmblem :
        //left_array -> left half of emblem (tied to variable emblem_left)
        //right_array -> right half of emblem (tied to variable emblem_right)
        //selected_face -> face/plane user selects for the sketch to be projected on
    function sketchEmblem(context is Context, id is Id, left_array is array, right_array is array, selected_face is Query) {
            var sketch1 = newSketch(context, id + "sketch1", {
                    "sketchPlane" : selected_face
            });
            for (var i = 0; i != 9; i += 1) {
                skFitSpline(sketch1, ("spline_a" ~ toString(i)), {
                            "points" : left_array[i]
                        });
            }
            for (var i = 0; i != 9; i += 1) {
                skFitSpline(sketch1, ("spline_b" ~ toString(i)), {
                            "points" : right_array[i]
                        });
            }
            skSolve(sketch1);
    }
    ...
    function extrudeEmblem(context is Context, id is Id, sketchId is string, selected_face is Query, depth is map) {
